...
class DataPassService {
     ...
     */
    async getAll(query) {
        const periods = await DataPassRepository.findAndCountAll(new QueryBuilder().addFromHttpRequestQuery(query));
    ...
    async getDataPassesPerPeriod(periodId, query) {
        ...
        const runs = await DataPassRepository.findAndCountAll(new QueryBuilder(baseClause).addFromHttpRequestQuery(query));
