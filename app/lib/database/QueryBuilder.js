...
class QueryBuilder {
	...
	build(params) {
		...
		const filteringPart = () => {
			const matchPhrase = matchParams
				.map(
					(filter) => `"${filter.queryParam}" LIKE '${filter.value}'`
				)
				.join(' AND ');

			const excludePhrase = excludeParams
				.map(
					({ queryParam, value }) =>
			...
			const fromPhrase = fromParams
				.map(({ queryParam, value }) => `"${queryParam}" >= ${value}`)
				.join(' AND ');

			const toPhrase = toParams
				.map(({ queryParam, value }) => `"${queryParam}" <= ${value}`)
				.join(' AND ');
		...
		const dataSubsetQueryPart = (params) =>
			params[DRP.countRecords] === 'true'
				? ''
				: `LIMIT ${params[DRP.rowsOnSite]} OFFSET ${
						params[DRP.rowsOnSite] * (params[DRP.site] - 1)
				  }`;
