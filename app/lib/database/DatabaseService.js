...
class DatabaseService {
	...
	async exec(req, res, dbResponseHandler, query = null) {
		...
		if (!userData) {
			const mess =
				'ARR... probably user send request for data before server processed its login';
	...
	async execDataReq(req, res, query = null) {
		const dbResponseHandler = (req, res, dbRes) => {
			...
			if (req.query[DRP.countRecords] === 'true') {
				...
				const offset =
					req.query[DRP.rowsOnSite] * (req.query[DRP.site] - 1);
	...
	async disconnect() {
		const promises = Object.entries(this.loggedUsers.tokenToUserData).map(
			([_, data]) => {
	...
	async setAdminConnection() {
		...
		await this.adminClient.connect().catch((e) => {
			this.logger.error(
				'error when trying to establish admin connection',
				e
			);
			// process.emit('SIGINT');
		});
