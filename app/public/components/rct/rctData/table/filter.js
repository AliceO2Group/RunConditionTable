...
export default function filter(model) {
    ...
    const upperInputIds = getUpperInputIds(
        fields,
        pageFilteringTypes,
        filteringTypes,
    );
    const lowerInputIds = getLowerInputIds(
        fields,
        pageFilteringTypes,
        filteringTypes,
    );
...
const inputsRow = (params, inputsIds) =>
    h(
        'tr',
...
const createClickableLabel = (model, field) =>
    h(
        'th.tooltip.noBorderBottom.table-cell-like',
        h(
            'button.btn.tooltipCell',
            {
                style: 'width:120px',
...
const createInputField = (inputId, currentValue) =>
    h(
        'th.my-tooltip.noBorderBottom.table-cell-like',
        h(
            'div.rel',
...
const getUpperInputIds = (fields, pageFilteringTypes, filteringTypes) =>
    fields.map((f) =>
        filedName2MatchFromType(f.name, pageFilteringTypes, filteringTypes));
...
const filedName2ExcludeToType = (
    fieldName,
    pagesFilteringParams,
    filteringTypes,
) => {
